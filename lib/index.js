...
function sendAndProcessResponse(plugins, req, done) {
  var k, err, res, timeoutId, checkIfDone, supportsLoadAndErrorEvents,
      xhr = req.xhr;
  ...
  checkIfDone = function(rawError) {
    ...
    processResponse(plugins, res, req, function(processedRes) {
      var error, response;
 ...
 */
function processResponse(plugins, res, req, done) {
  var fn, send, processors, plugin, current, total, next;

  // We pass this function in to the plugin, in case it needs to trigger
  // another send.
  send = function(newReq) {
    if (!(newReq instanceof Request)) {
      // FIXME: how do we get the defaults here?
      var defaults = {};
      newReq = createAndProcessRequest(plugins, newReq, defaults);
    } else {
      newReq = processRequest(plugins, newReq);
    }

    newReq.xhr = createXHR(plugins, newReq);
    sendAndProcessResponse(plugins, newReq, done);
  };
  ...
  next = function(partiallyProcessedRes) {
    ...
    // use a pass-through thunk to continue processing.
    if (typeof fn !== 'function') {
      fn = function(continuation) {
        continuation(partiallyProcessedRes);
      };
    }
    ...
    // Execute the process
    if (current < total) {
      // If there are still processors to be run,
      // then set `next` up to run when this processor finishes.
      fn(next, req, send);
    } else {
      // Otherwise, there are no processors left to be run,
      // so set `done` up to run when this processor finishes.
      fn(done, req, send);
    }
  };
